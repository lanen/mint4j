...
public abstract class GenericTypeResolver {
	 ...
	 */
	/*public static Type getTargetType1(MethodParameter methodParam) {
		Validate.notNull(methodParam, "MethodParameter must not be null");
	 ...
	 * the given target method which is assumed to return the given interface or
	 ...
	 *         <code>null</code> if not resolvable or if the single argument is
	 *         of type {@link WildcardType}.
	 */
	public static Class<?> resolveReturnTypeArgument(Method method,
		...
		if (returnType.equals(genericIfc)) {
			if (genericReturnType instanceof ParameterizedType) {
				...
				Type[] actualTypeArguments = targetType
		...
		return GenericTypeResolver.resolveTypeArgument((Class<?>) returnType,
				genericIfc);
	}
	...
	private static Class[] doResolveTypeArguments(Class ownerClass,
		...
		while (classToIntrospect != null) {
			if (genericIfc.isInterface()) {
				...
				for (Type ifc : ifcs) {
					Class[] result = doResolveTypeArguments(ownerClass, ifc,
							genericIfc);
					if (result != null) {
						return result;
					}
	...
	private static Class[] doResolveTypeArguments(Class ownerClass, Type ifc,
		...
		if (ifc instanceof ParameterizedType) {
			...
			if (genericIfc.equals(rawType)) {
				...
				return doResolveTypeArguments(ownerClass, (Class) rawType,
						genericIfc);
			}
	...
	private static Class extractClass(Class ownerClass, Type arg) {
		if (arg instanceof ParameterizedType) {
			return extractClass(ownerClass,
